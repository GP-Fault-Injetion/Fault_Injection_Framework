...
/* Definition of the hardware abstraction function table */
typedef struct {
    Std_ReturnType (*Read)(uint8 DeviceIndex, uint16 BlockNumber, uint16 BlockOffset, uint8 *DataBufferPtr, uint16 Length);
    Std_ReturnType (*Write)(uint8 DeviceIndex, uint16 BlockNumber, uint8 *DataBufferPtr);
    Std_ReturnType (*EraseImmediateBlock)(uint8 DeviceIndex, uint16 BlockNumber);
    Std_ReturnType (*InvalidateBlock)(uint8 DeviceIndex, uint16 BlockNumber);
    void (*Cancel)(uint8 DeviceIndex);
    MemIf_StatusType (*GetStatus)(uint8 DeviceIndex);
    MemIf_JobResultType (*GetJobResult)(uint8 DeviceIndex);
} MemIf_MemHwA_FunctionsType;
